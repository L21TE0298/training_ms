...
public class CouponController {
	 ...
	 * = @Schema(implementation = Coupon.class))) })
	 * 
	 * @GetMapping
	...
	@GetMapping("{idCoupon}")
	public ResponseEntity<?> getByidCoupon(@PathVariable @Min(value = 1 , message="The required request parameter idCoupon is negative our missing.") Integer idCoupon) {
	...
	@ApiResponse(responseCode = "200", description = "Registered coupon", content = {
			@Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = Coupon.class))) })
	@PostMapping
	public ResponseEntity<Map<String, Object>> register(@Valid @RequestBody CouponDTO couponDTO) {
		CouponDTO saveCoupon = convertToDTO(service.save(convertToEntity(couponDTO)));
		Map<String, Object> response = new HashMap<>();
		response.put("message", "Coupon registered successfully");
		response.put("coupon", saveCoupon);
	...
	@PutMapping("{idCoupon}")
	public ResponseEntity<?> update(@Valid @RequestBody Coupon coupon, @PathVariable @Min(value = 1 , message="The required request parameter idCoupon is negative our missing.") Integer idCoupon) {
